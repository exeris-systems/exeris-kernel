...
- **Server push is SSE; full duplex is WebSocket, and it is not started for you** — one-directional `HttpStreamExchange` since 0.10 ([ADR-043](../adr/ADR-043-kernel-http-streaming-spi.md)) and `WebSocketExchange` since 0.12 ([ADR-084](../adr/ADR-084-websocket-provider-spi.md)). The WebSocket engine is embeddable rather than bootstrapped: `KernelBootstrap` starts no WebSocket subsystem, so a deployment that wants one constructs the engine and supplies a handshake handler. It has **no server-initiated keepalive** — the read path parks without a timeout, so liveness detection is the client's. The SSE body is HTTP/1.1 close-delimited today, which matters for reverse proxies that buffer length-unknown responses — see [`subsystems/http.md`](../subsystems/http.md).
