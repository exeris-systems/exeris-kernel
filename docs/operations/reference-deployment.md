---
title: "Exeris Kernel — Reference Deployment (single-node Community)"
type: operations
visibility: public
...
- **PostgreSQL 16** — the version most Persistence, Flow snapshot store, and transactional outbox integration tests target via Testcontainers. Pooled via HikariCP. The RowCursor type-rendering contract (ADR-080) is separately validated against PostgreSQL 17 — see [`../rowcursor-type-set.md`](../rowcursor-type-set.md).
...
| **Profile** | `EXERIS_KERNEL_PROFILE=prod` env var, or `exeris.kernel.profile=prod` system property (`PROD` is the default when neither is set) — opaque error codes; DEV exposes stack traces, so keep PROD in deployment |
...
If you build on the box, Maven needs a JDK matching the build's own baseline.
...
**Boot SLO** — the whitepaper's contract table states a design target of cold-start **P99 ≤ 500 ms (Community) / ≤ 800 ms (Enterprise)** ([`../whitepaper.md`](../whitepaper.md)), observable via the emitted `KernelBootReady` JFR event; no in-repo test asserts that threshold, and the reference Kubernetes manifest itself budgets up to 150 s for cold start (see below). Do **not** set a nonzero probe `initialDelay`; use `initialDelaySeconds: 0` plus a `startupProbe` (the manifest in [`bootstrap.md`](../subsystems/bootstrap.md) budgets a generous cold-start window via `startupProbe` while readiness/liveness poll from t=0).
...
- **Server push is SSE; full duplex is WebSocket, and it is not started unless you ask** — one-directional `HttpStreamExchange` since 0.10 ([ADR-043](../adr/ADR-043-kernel-http-streaming-spi.md)) and `WebSocketExchange` since 0.12 ([ADR-084](../adr/ADR-084-websocket-provider-spi.md)). The WebSocket engine is reachable two ways since 0.12: embeddable — a deployment constructs it and supplies a handshake handler, with no kernel boot — or through the `websocket` subsystem that `KernelBootstrap` starts. The subsystem is **opt-in**, `websocket.enabled` defaulting to `false`, so an upgrade opens no listener (ADR-084 A2). It has **no server-initiated ping** — `keepAliveIntervalMillis` on `WebSocketConfig` is accepted but not honoured by the Community provider, so nothing is sent to hold a NAT/proxy path open through a quiet period (a client's own pings are answered, per RFC 6455 §5.5.2). A dead peer is still reclaimed server-side: the carrier's idle timeout (`websocket.idleTimeoutMillis`, default 60&nbsp;s) closes a connection that has moved no bytes for that long. The SSE body is HTTP/1.1 close-delimited today, which matters for reverse proxies that buffer length-unknown responses — see [`subsystems/http.md`](../subsystems/http.md).
