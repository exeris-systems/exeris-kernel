...
## Latest Run Commands (2026-03-22)

- `mvn -pl exeris-kernel-core -DskipTests clean package`
- `java --enable-preview -cp "exeris-kernel-core/target/test-classes:exeris-kernel-core/target/classes:$(cat exeris-kernel-core/target/test-classpath.txt)" org.openjdk.jmh.Main CoreContinuationLocalityBaseline -f 2 -wi 5 -i 8 -tu s`
- `mvn -pl exeris-kernel-core -Dtest=CorePaqsSchedulerTckTest test -DfailIfNoTests=false`
- `mvn -pl exeris-kernel-community -Dtest=CommunityPaqsExecutionBackendContractTest test -DfailIfNoTests=false`
...
Validation blocker note (environment): full module test sweeps (`mvn -pl exeris-kernel-core test` and `mvn -pl exeris-kernel-community test`) currently fail at discovery stage with `UnsupportedClassVersionError` preview mismatch (class file `69.65535` vs runtime expecting `70.65535`). This is treated as an environment/toolchain blocker, not a seam contract failure.
