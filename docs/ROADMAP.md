...
**Merge Gate:** a real-boot e2e drives a router over a socket and asserts a by-id `GET`/`PUT`/`DELETE` *resolves* (not 404) and a `POST` with a JSON body *decodes* (not 500); the `HttpRouter` test gains a path-parameter case; existing exact/prefix routing and SSE stream-route resolution unregressed.
