...
**Milestone:** **v0.10** (ADR-046, reserved 2026-06-29; content DRAFT in `docs/adr/`), sequenced with the sibling Events `topic` item above. The kernel SPI + Community `CommunityJsonEventPayloadCodec` + `AbstractEventPayloadCodecTck` + Community binding land first (the `EVENT_PAYLOAD_CODEC_REGISTRY` slot ships defined-but-unbound, the `EVENT_STREAM_*` precedent); the bootstrap slot-binding + the `exeris-tooling` `KernelEventGenerator` publisher rewrite follow lockstep.

See also: [Events Subsystem](./subsystems/events.md); `docs/adr/ADR-046-…` (DRAFT); ADR-036 (the request-decode quadrant this mirrors, incl. site-B resolution); the sibling Events `topic` entry above.
