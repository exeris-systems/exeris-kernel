...
**Gap:** every HTTP test in this repository runs in one JVM. Three tests use `ProcessBuilder` and all three spawn `openssl req` to *generate a certificate* — none stands up a peer. So the combination an operator actually deploys, two kernels on a socket, is asserted nowhere here.
