...
**Status (v0.10):** **kernel SPI DELIVERED** (ADR-046, Accepted; PR #222) — `EventPayloadCodec` / `EventPayloadCodecRegistry` / `EventCodecContext` in `eu.exeris.kernel.spi.events.codec`, Community `CommunityJsonEventPayloadCodec`, `AbstractEventPayloadCodecTck` + Community binding (21 cases green), `KernelProviders.EVENT_PAYLOAD_CODEC_REGISTRY` slot + accessor. Strictly additive — `EventBus`/`EventEngine`/`EventPayload` unchanged; the slot ships defined-but-unbound (the `EVENT_STREAM_*` precedent). Sequenced with the sibling Events `topic` item. **Lockstep follow-ups:** bootstrap slot-binding + the deferred JFR encode-failure/codec-resolution event (PR-A.1), and the `exeris-tooling` `KernelEventGenerator` `*EventPublisher` rewrite (PR-B).
