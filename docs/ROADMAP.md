...
**Status (v0.11): RFC ACCEPTED — shape selected, implementation post-1.0.** [`RFC-2026-06-29`](rfc/RFC-2026-06-29-webclient-service-addressing.md) selects **option (c), a `ServiceResolver` SPI seam**, with (a) static map and (b) DNS-SRV as its two first-party Community drivers and (d) the mesh case reframed as a pass-through driver rather than a kernel non-feature. Its premises were re-verified at acceptance six weeks after drafting and still hold — `KernelWebClient` is still single-host and no resolver surface exists. Dissent recorded: option (e) do-nothing stays live if T12 does not materialise, since C's justification rests on its two in-repo drivers supplying the contract pressure that a single external consumer would otherwise have to.
