...
2. **`noExecutorsAnywhere`'s stated reason is stale** — "All concurrency must use `StructuredTaskScope` (JEP 525)" now contradicts the Platform Baseline directly above it. The rule stays; the text needs correcting alongside the finding above.
...
**Remaining (S19):** substitute the four sites, flip the build to JDK 25 with no `--enable-preview`, and add the preview-bytecode scan to CI.
