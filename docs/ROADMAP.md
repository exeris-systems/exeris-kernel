...
Resolved with rules rather than with skips: `pmd-ruleset-tck.xml` and `checkstyle-tck.xml` are the kernel rulesets minus a named, justified list, so a rule added upstream still applies here unless somebody excludes it on purpose. Ten PMD rules were deliberately kept live and their findings fixed in the code. The L0 ban regexes — ThreadLocal, unstructured concurrency, `Unsafe`, direct `Arena` allocation, DI, `java.util.Date` — are all retained, and the six sites where a TCK message *names* a banned type carry a local suppression rather than the rule being weakened. Coverage is the one structural exemption, alongside the diagnostics CLI's existing one.
