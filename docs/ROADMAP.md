...
**Gate:** `tools/preview-bytecode-scan/` reads `*/target/classes` and fails on any distributed class carrying `minor_version 0xFFFF` or a class-file major above the LTS baseline — bytecode rather than a source grep, because the stamp is what a consumer trips over and it survives generated code a grep would miss. Proven to fail on both modes by byte-level mutation. Current state: **930 distributed classes, major 69, zero preview-stamped**; 128 test classes remain preview-stamped and are not published.
