...
**Status (v0.11):** SPI + Community driver + `AbstractJobSchedulerTck` (18 cases) landed; contract doc at `docs/subsystems/scheduling.md`. Bootstrap wiring (`KernelProviders.JOB_SCHEDULER_PROVIDER` / `JOB_SCHEDULER`, config keys, lifecycle) is the remaining slice.
...
**Status (v0.11): IN PROGRESS — acceptance step 1 is closed by measurement, step 2's GA layer has landed, the four sites are not yet substituted.** Surfaced 2026-06-18 from the JEP 401 (Valhalla → JDK 28 *preview*) + `StructuredTaskScope` GA-timeline analysis. This is a **1.0-GA-blocking** baseline: until the default critical path is preview-clean on a GA LTS, the word "GA" cannot honestly be applied. Distinct from the per-feature roadmap items above because it constrains the *distribution contract*, not a single subsystem. Decision record: **ADR-066** (reserved 2026-08-08).

**What the acceptance probe actually found (2026-08-08).** Step 1 above asked for "a clean no-`--enable-preview` compile of the default reactor" as the definitive confirmation that `StructuredTaskScope` is the only preview dependency. That has now been run on a real JDK 25 LTS rather than reasoned about, and the two blockers turned out to be separable and very unequal:

- **The JDK-26 target costs nothing.** `mvn clean install` at `--release 25` on JDK 25.0.3 is green across all eleven modules, 747 tests. No JDK-26-only API is used anywhere in the tree, so the `26` in the build was never load-bearing. What it does cost is three properties and four constants: `maven.compiler.release` in the root and `exeris-kernel-build-config` poms, `@SupportedSourceVersion(SourceVersion.RELEASE_26)` in both `build-config` annotation processors, and a hardcoded `--release 26` in two processor tests. Those four fail the build *before* the preview errors appear, which is worth naming — they are the misleading first symptom for anyone attempting this without measuring.
- **The preview flag is the whole of the work,** and it is exactly the four known sites. `exeris-kernel-spi` compiles clean without the flag, which upgrades "the SPI is already preview-clean for STS" from a grep result to a compiler result.

**A preview leak that no signature audit could have found.** `StructuredTaskScope.open()` — the bare form used by `SubsystemOrchestrator.startParallel` — defaults to `Joiner.awaitAllSuccessfulOrThrow()`, so `join()` throws `StructuredTaskScope.FailedException`, an unchecked **preview** class (verified by running it, not inferred from the javadoc). `FOUNDATION` starts sequentially and every other phase starts in parallel, so a mandatory subsystem failure in any L1 phase surfaces to callers as a preview type; `CoreFailurePolicyTckTest:113` already catches `BootstrapException \| StructuredTaskScope.FailedException` for exactly that reason. This is preview in the *observable failure behaviour*, not merely in shipped bytecode, and neither of the two binary acceptance checks in the merge gate would have seen it. It also means `startParallel`'s post-join failure-collection block — the one that assembles `BootstrapException("N subsystem(s) failed in phase X")` — is unreachable whenever a subtask fails, so the substitution makes it live for the first time rather than preserving current behaviour.

**Landed (S18):** `eu.exeris.kernel.core.concurrent.StructuredScope` — owner-confined, await-all `fork`/`join`/`cancel` on virtual threads, with `ScopedValue` bindings handed in as a `ScopedValue.Carrier` and re-established inside each task. There is deliberately no no-argument `open()`: `StructuredTaskScope` forks inherit bindings, a plain virtual thread does not (`isBound()` reads `false` inside `Thread.ofVirtual().start(...)` and `true` inside a forked subtask — measured on JDK 25), and no GA API can snapshot a thread's live bindings, so a silently-inheriting drop-in cannot be written. `openWithoutBindings()` names the choice instead of hiding it. Additive; no call site changed yet. Proven non-vacuous by mutation: dropping carrier propagation reddens both binding cases and nothing else, and removing owner confinement reddens exactly the confinement case. The straggler case initially **survived** its mutant — asserting "was interrupted" and "no longer RUNNING" passes just as well against a `close()` that never waits, since the task usually wins the race — and was rewritten to hold the task busy after the interrupt, which is what makes the structured guarantee testable.

**Remaining (S19):** substitute the four sites, flip the build to JDK 25 with no `--enable-preview`, and add the preview-bytecode scan to CI. Carried with it, deliberately not done in S18: the remaining `StructuredTaskScope (JEP 525)` references that S19 — not S18 — will falsify, in `CONTRIBUTING.md`, `docs/whitepaper.md`, `docs/architecture.md`, `ADR-007`, and in-code javadoc on `SubsystemOrchestrator`, `OutboxOrchestrator` and `EventLoop`. S18 corrected only what S18 itself made false (the two ArchUnit rationales); an additive slice that changes no call site does not yet contradict a document describing how the four sites work. One of those is wrong on its own terms and worth fixing whatever else happens to it: `SubsystemOrchestrator:653` reads "JEP 525 — stable in JDK 24", and `StructuredTaskScope` has never been stable in any JDK.
