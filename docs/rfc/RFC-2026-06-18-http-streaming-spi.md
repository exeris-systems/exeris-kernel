...
- **New SPI surface** → requires architect sign-off + an `AbstractHttpStreamExchangeTck` for the streaming contract (open / emit-N / close / disconnect, backpressure, no respond-once regression) with a Community binding; the Enterprise overlay is declared as a cross-repo obligation.
...
8. **ADR-012 JWT expiry mid-stream (fail-closed).** The respond-once path handles token expiry trivially — the request is already complete. A held-open stream authenticated at T=0 whose JWT expires at T+n is a failure mode that path never has, and ADR-012 §5 is unambiguous (deterministic deny, no fail-open fallthrough). ADR-043 must define the re-validation contract (validated once per-connection at open / periodic-heartbeat / per-emit) and the close/error behaviour on expiry (stream close with an `EX-HTTP-*` code vs silent drop). Leaving it open yields either a silent fail-open (ADR-012 violation) or an unspecified close — load-bearing under the same criterion as the items above.
