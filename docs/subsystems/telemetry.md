...
| `CommunityBlobFailures.BlobCeilingExceededEvent` *(eu.exeris.kernel.storage.BlobCeilingExceeded; community module, pkg-private, since v0.11 — ADR-056)* | A driver refuses a transfer larger than its configured single-object ceiling. A refusal by policy rather than a failure, which is why it is its own event and not a `BlobTransferFailed` with a special reason. | `providerName`, `operation`, `container`, `declaredBytes`, `ceilingBytes` |
| `CommunityJobJfrEvents.JobDispatchEvent` *(eu.exeris.kernel.scheduling.JobDispatch; community module, pkg-private, since v0.11 — ADR-057 §8)* | The scheduler dispatches a job onto a virtual thread. Single-phase commit — contractual here, since a scheduler is the subsystem most exposed to the VT straddle. | `schedulerName`, `jobName`, `jobId` |
| `CommunityJobJfrEvents.JobCompletionEvent` *(eu.exeris.kernel.scheduling.JobCompletion; community module, pkg-private, since v0.11 — ADR-057 §8)* | A dispatched job body returns normally. Duration measured with plain nanos and attached at commit, never spanning `begin()`/`commit()`. | `schedulerName`, `jobName`, `jobId`, `durationNanos` |
| `CommunityJobJfrEvents.JobFailureEvent` *(eu.exeris.kernel.scheduling.JobFailure; community module, pkg-private, since v0.11 — ADR-057 §5/§8)* | A job body throws (`EX-JOB-9003`, reason = exception class), or a dispatch is refused because the submission captured no identity context (`EX-JOB-9001`, reason = `no-captured-context`). The error code is what distinguishes a kernel refusal from a crash — at the SPI level both look identical. | `schedulerName`, `jobName`, `jobId`, `errorCode`, `reason` |
