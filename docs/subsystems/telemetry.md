...
| `CommunityBlobFailures.BlobTransferFailedEvent` *(eu.exeris.kernel.storage.BlobTransferFailed; community module, pkg-private, since v0.11 — ADR-056)* | A Community blob store fails an I/O operation (init / upload / download / stat / delete). Emitted from the same choke point that builds the exception, so the two cannot drift. | `providerName`, `operation`, `container`, `exceptionClass`, `exceptionMessage` |
