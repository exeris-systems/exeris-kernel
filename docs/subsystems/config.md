...
| `websocket.bindHost`                               | `string`  | `127.0.0.1`         | ❌ IMMUTABLE | ✅ WIRED    | Listen address. Loopback by default, so enabling the subsystem without choosing an address does not publish it to the network |
...
| `websocket.keepAliveIntervalMillis`                | `long`    | `20000`             | ❌ IMMUTABLE | ✅ WIRED    | Ping interval (`WebSocketConfig.DEFAULT_KEEP_ALIVE_INTERVAL_MILLIS`) |
