...
**An empty `lookupParked` is not permission to drop the wake (since 0.12).** `lookupParked` reports a live, *running* instance as absent on purpose: handing it out as parked would let a second event schedule the same flow again (`AbstractFlowSchedulerTck.deferredWakeAfterSelfParkClearsParkedRegistration`). But a choreographed callback can arrive while the instance is still inside the step that is about to `PARK`, and that absence is indistinguishable from a stale key. `FlowChoreographyBridge` therefore delivers the wake on the empty branch too, as a `wake()` carrying `PARKED` intent: `resolveParkedInstance` admits the running instance, `beginScheduleAfterWake` defers it through `wakePending`, and `honourDeferredWake` resumes it when the run exits. A genuinely stale key still fails `NOT_PARKED` and is absorbed. Dropping it instead stranded the saga permanently, since a choreography wake is one event per business trigger and nothing re-sends it.
