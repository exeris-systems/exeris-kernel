...
| `FlowSchemaMismatchEvent` (since 0.10) | `eu.exeris.kernel.flow.SchemaMismatch` | `CoreFlowRuntime.validateSnapshotStepBounds()` — emitted single-phase immediately before a parked-saga resume is rejected fail-closed because the persisted step no longer indexes a step in the active definition (`EX-FLOW-7002 / phase=SCHEMA_MISMATCH`) | `engineName`, `definitionName`, `instanceIdMost`, `instanceIdLeast`, `persistedStep`, `planStepCount` |
