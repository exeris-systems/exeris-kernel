...
| **Saga parked before the version column existed** | **Rejected fail-closed** (`reason="DEFINITION_VERSION_ABSENT"`). The durable column is nullable with **no default**: backfilling 1 would assert that every parked saga belongs to the first version, which is the guess this epic exists to stop making. Same remedy as the identity row above — drain in-flight Sagas before upgrading. |
