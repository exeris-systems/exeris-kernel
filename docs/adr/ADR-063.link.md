---
title: "ADR-063 — `ExerisHttpSecurity`: Spring-shaped route rules compile onto the kernel's decision (link stub)"
type: adr-link
visibility: public
...
spring-runtime decision the kernel keeps awareness of. Neither has an authoritative copy in this
repository; both are listed in the global registry, `exeris-docs/adr-index.md`, and both now carry a
stub here — [ADR-030's](./ADR-030.link.md) was written in v0.12 on the strength of this paragraph
calling the two the same shape while only one of them had one.
