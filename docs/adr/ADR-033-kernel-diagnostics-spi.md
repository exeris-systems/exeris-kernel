...
> **Amendment (v0.9.x, pre-1.0) — `listCapabilities()` removed from the surface.** The originally
> accepted surface named `listCapabilities() → CompositionSnapshot` as a window onto the ADR-024
> `@Provides`/`@Requires` composition graph. The v0.9 implementation exposed the design flaw: ADR-024 is a
> **build-time** validation model — by the time anything runs on the kernel the composition is already
> resolved and erased into providers/subsystems, so a *runtime* SPI structurally cannot read it back. The
> Community implementation could only fabricate descriptors by re-reading the same `SUBSYSTEMS` slot that
> `getBootstrapDag()` reads, emitting one `CapabilityDescriptor` per subsystem that "provides" only
> itself — strictly a relabelled, lossy subset of `getBootstrapDag()`, with the Javadoc promising a
> composition graph it could not deliver. That is the worst failure mode: a Tier-1 contract made
> lexically capability-aware while being substantively incapable of it. **Resolution:** remove
> `listCapabilities()`, `CompositionSnapshot`, and `CapabilityDescriptor` from `eu.exeris.kernel.spi.diagnostics`,
> and retire audit code `EX-DIAG-1002`. Zero information is lost (the data was a subset of
> `getBootstrapDag()`), and the SPI stops importing a Tier-2 vocabulary it cannot honour. Real composition
> introspection (the `@Provides`/`@Requires` graph for Caps/SKU work) belongs **build-time (tooling) /
> manifest-side (platform)**, not in the runtime kernel — see the corresponding correction to the bridge's
> `kernel:*` surface (ADR-025, separate repo). Done pre-SPI-freeze: the surface is documented as closing at
> 1.0 GA, so this lands inside the window before it is frozen. The amendment supersedes the original
> "Interface surface", Obligations 1 & 9, the ADR-024 cross-reference, and EP step 8 below.
...
public interface KernelDiagnostics {
    ...
    // + getJvmErgonomics() (default) — added v0.9; see Obligation 9.
    // listCapabilities() removed pre-1.0 — see the v0.9.x amendment above.
}
...
1. **SPI package.** All public types live in `eu.exeris.kernel.spi.diagnostics.*`. Snapshot records (`ProvidersSnapshot`, `BootstrapDagSnapshot`, `SubsystemSnapshot`, `RuntimeErgonomicsSnapshot`, plus their nested descriptors) are sealed where the kernel knows the exhaustive set; otherwise plain `record`. *(Amended v0.9.x: `CompositionSnapshot` / `CapabilityDescriptor` removed with `listCapabilities()` — see the amendment banner.)* No type in this package may import from `exeris-kernel-core`, `org.springframework.*`, `io.netty.*`, `reactor.*`, `jakarta.servlet.*`, or any host-runtime-specific package.
...
9. **TCK obligation.** `AbstractKernelDiagnosticsTck` ships in `exeris-kernel-tck` with the SPI. It exercises the four-method surface against a known-fixture kernel (records returned shape, `schemaVersion`, `Optional<>` field semantics, snapshot non-atomicity acknowledged). *(Amended v0.9: four-method → five-method when `getJvmErgonomics() → RuntimeErgonomicsSnapshot` was added; then five → four (v0.9.x) when `listCapabilities()` was removed — see the amendment banner. Per Obligation 5 the new method is a binary-breaking interface change, so it ships as a `default` returning `RuntimeErgonomicsSnapshot.unknown()` — all known providers updated in lockstep, the TCK gaining the well-formedness / `Optional.empty()`-degradation / `default`-method-compat cases. The wire schema grew append-only — a new top-level record, `schemaVersion` held at `1.0` since v0.9.0 ships the whole surface as the first published schema.)* Community provider in `exeris-kernel-community-testkit` runs the TCK in CI; Enterprise provider runs the same TCK plus overlay-specific cases in `exeris-kernel-enterprise` CI (ADR-008 open-core loading symmetry).
...
- **ADR-024** (Capability Composition Model — `@Provides` / `@Requires` / Build-Time Validation) — *(superseded by the v0.9.x amendment)* originally cited as the source for `listCapabilities()`. The flaw was exactly here: ADR-024 is **build-time** validation, resolved and erased before runtime, so a runtime SPI cannot return its composition graph. Composition introspection stays build-time (tooling) / manifest-side (platform); the runtime SPI exposes only the resolved bootstrap topology via `getBootstrapDag()`.
