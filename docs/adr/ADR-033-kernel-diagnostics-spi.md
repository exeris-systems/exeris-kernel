...
4. **CLI artefact.** New module `exeris-kernel-diagnostics-cli` in the `exeris-kernel-community` reactor, shaded executable JAR with stable `main` class `eu.exeris.kernel.diagnostics.cli.DiagnosticsCli`. Released in lockstep with the kernel.
