---
title: "ADR-036: Server-Side Request Body Decoder SPI — `HttpRequestBodyDecoder` + Generated-Handler Resolution"
type: adr
visibility: public
...
Bootstrap source = `CommunityHttpProvider.requestBodyDecoderRegistry()`. The generated handler reads the registry via `HttpKernelProviders.httpRequestBodyDecoderRegistry()`. This **closes the request-decode half of ADR-034 §322's deferred open item** ("a future ADR may opt to read the `ScopedValue` slot") — the server side is exactly where the generated handler has no construction seam of its own, so reading the slot is the natural binding. (The client-decode half of §322 — auto-binding `KernelWebClient` from the slot — remains deferred; this ADR does not touch it.)

**Binding seam (where the slot is *bound*, not just read).** The registry is bound into the **kernel-scope carrier** in `CommunityHttpSubsystem.providerBindings()` — the same `UnaryOperator<ScopedValue.Carrier>` channel that already binds `HTTP_SERVER_ENGINE`, `HTTP_PROVIDER`, and `HTTP_CLIENT_ENGINE` (this is the first HTTP *registry* ScopedValue to use it). `SubsystemOrchestrator` composes that operator into the single carrier `KernelBootstrap` runs the kernel main inside, so the binding is inherited by every virtual thread in the kernel scope — including per-request dispatch threads where generated handlers run. It is therefore **not** a per-request binding: the slot is live for the whole runtime scope, and a handler reading it at construction *or* per-call both resolve. Binding is `ifPresent`-guarded and scoped to the server branch (a CLIENT-only kernel has no inbound handlers, so it contributes no decoder). The server-side response *encoder* registry stays constructor-threaded (`createServerEngine(config, ENCODER_REGISTRY)`) and is untouched — only request-decode needs the slot because the generated handler has no kernel-provided construction seam. TOOL-139 (the generator) MUST NOT assume a narrower per-request binding.
