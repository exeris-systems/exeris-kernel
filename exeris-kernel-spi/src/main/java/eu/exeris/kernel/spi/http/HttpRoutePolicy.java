 ...
 *
 * <p>A route policy is a property of the application's own URL space — which of its paths are
 * public, which demand an identity, which demand a scope — so the application is what states it.
 * {@link #requirementFor(HttpMethod, String)} is consulted once per request, before the handler
 * runs, and the answer decides admission.
 *
 * <p><b>Allocation:</b> zero-alloc on hot path — a policy answers with pre-built
 * {@link RouteRequirement} instances and allocates nothing per request
 * <p><b>Thread confinement:</b> any thread — {@code requirementFor} runs on the admission path of
 * every request, concurrently
 *
 * @implSpec An implementation of {@link #requirementFor(HttpMethod, String)} is:
 *           <ul>
 *             <li><b>Total.</b> Every {@code (method, path)} pair gets an answer, and never
 *                 {@code null} — see {@link #unmatched()} for how to say "nothing declared for this
 *                 route", which is a decision rather than an absence. A {@code null} is treated as
 *                 a policy defect and denies the request outright; it is <em>not</em> read as
 *                 {@link #unmatched()}, because that would let a broken policy admit any
 *                 authenticated caller to a route it never described.</li>
 *             <li><b>Pure and thread-safe.</b> It runs on the admission path of every request,
 *                 concurrently.</li>
 *             <li><b>Allocation-free.</b> Return pre-built {@link RouteRequirement} instances;
 *                 building one per request puts allocation on the admission path, which the
 *                 performance contract does not allow for the neighbouring RBAC decision either
 *                 (ADR-014 §5).</li>
 *           </ul>
 * @apiNote <b>The unmatched route is the dangerous case.</b> A policy that declares rules for
 *          {@code /api/orders/**} says nothing about {@code /api/internal}, and what happens to
 *          that request is the whole security posture. This contract does not choose for you, but
 *          it does force you to choose: {@link #unmatched()} names the two options, and a policy
 *          returns one of them rather than leaving the question implicit. Answering
 *          {@link RouteRequirement#permitAll()} for unmatched routes is fail-open, and must be a
 *          decision the application makes knowingly.
 *          <p>Bind the policy into {@link HttpKernelProviders#HTTP_ROUTE_POLICY} and read it back
 *          through {@link HttpKernelProviders#httpRoutePolicy()}. While the slot is unbound the
 *          kernel applies no per-route requirement at all.
 * @since 0.11
 */
@FunctionalInterface
public interface HttpRoutePolicy {
     ...
     * @apiNote <b>Before choosing fail-closed, account for the routes you did not write.</b> A
     *          deployment that binds a policy but no {@code HTTP_SERVER_HANDLER} gets the driver's
     *          built-in liveness, readiness and database-probe endpoints — routes like any other,
     *          so a fail-closed unmatched answer denies them and an orchestrator's probes start
     *          failing against a healthy process. Declare them
     *          {@link RouteRequirement#permitAll()} explicitly. The kernel does not exempt them for
     *          you: a driver-local notion of "public" would be a second answer to the question this
     *          contract owns, and the first thing it would do is disagree with yours.
     */
     ...
     *
     * <p>The list is copied once here, so the folded policy allocates nothing per request.
     *
     ...
     * @implSpec Inside a composition a policy is total over <em>its own</em> routes and abstains
     *           elsewhere; the composition is what is total. A policy bound directly to the
     *           provider slot is still total on its own, because nothing folds for it — an
     *           abstention reaching the decision point is a defect and denies. A {@code null} from
     *           a composed policy likewise stays a defect and ends the fold: reading it as a
     *           quieter abstention would let a broken policy hand its routes to the next one
     *           instead of denying them.
     * @apiNote {@code whenNoneDeclares} is a required argument because it is the
     *          fail-open/fail-closed choice {@link #unmatched()} names, and the reason abstention
     *          exists is that a second author must not answer it. A default here would re-create
     *          the problem it solves: a composed policy silently overruling the stance the
