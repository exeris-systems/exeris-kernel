...
- **Identity Classes for Data**: Banned. Domain values (e.g., configurations, pointers, limits) must be `value record` or `value class` to enable heap flattening (JEP 401).
- **Reflection / Spring / DI Frameworks**: Banned. We use strict Zero-Magic DI (constructors) and `ServiceLoader` for the Open-Core SPI mechanism.
- **Leaky Abstractions**: Banned. High-level logic (e.g., `CitadelRepository`) must never import JDBC, HikariCP, or `io_uring` specific classes. They must depend strictly on `exeris-kernel-spi`.
