    ...
    class GovernorZoneTransitions {
        ...
        void utilizationDropsAfterRelease() {
            ...
            assertThat(utilization)
                    .as("utilization() MUST drop to <= 0.70 (NORMAL zone) after all buffers closed. Got: %.4f — governor would incorrectly throttle at idle", utilization)
    ...
    class FixedBudgetPrecision {
        ...
        void utilizationZeroAtIdle() {
            ...
            assertThat(allocator.stats().utilization())
                    .as("utilization() MUST be exactly 0.0 at idle for fixed-budget allocators — governor must not trigger WARNING on empty pool")
                    .isZero();
        }
        ...
        void utilizationNeverExceedsOneUnderSaturation() {
            ...
            try {
                for (int i = 0; i < count; i++) {
                    ...
                    double u = allocator.stats().utilization();
                    assertThat(u)
                            .as("utilization() MUST NEVER exceed 1.0. Got: %.4f after %d allocations — WatermarkManager.forUtilization() uses this to classify levels, values > 1.0 corrupt the classification", u, i + 1)
                            .isLessThanOrEqualTo(1.0);
                }
            } finally {
                held.forEach(LoanedBuffer::close);
