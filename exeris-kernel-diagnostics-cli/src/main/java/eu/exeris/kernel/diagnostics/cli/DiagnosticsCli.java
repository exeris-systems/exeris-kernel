...
public final class DiagnosticsCli {
     ...
     *
     * <p>Most of what can go wrong arrives as one type, and what does not is worth knowing.
     * {@link KernelBootstrap#inspect(Runnable)} runs this method's body inside its own
     * {@code try}, whose last clause catches {@link Exception} and rewraps it, so a missing
     * provider and a broken stdin both reach a caller as {@code BootstrapException} with the
     * original as {@link Throwable#getCause()} — never as themselves. Branch on the cause, not
     * on the type.
     *
     * <p>That clause catches {@link Exception}, not {@link Throwable}, so an {@link Error} walks
     * through it untouched. A provider that is <em>present</em> but fails to initialise is
     * exactly that case: {@link ServiceLoader} raises {@link ServiceConfigurationError}, which
     * {@link #loadDiagnostics()} does not translate, and it leaves this method unwrapped and
     * undeclared. So an absent provider and a broken one — the two failures an operator is most
     * likely to cause — surface in different shapes.
     *
     * @param args ignored — the CLI takes no options and its whole input is the request stream
     * @throws KernelBootstrap.BootstrapException if config or topology resolution fails before
     *         the first request is read; or, wrapping the cause, if no
     *         {@link KernelDiagnosticsProvider} is on the classpath
     *         ({@link IllegalStateException}), or reading stdin or writing stdout fails
     *         mid-session ({@link java.io.UncheckedIOException} over the {@link IOException}).
     *         A failure inside one request never gets this far: {@link #handle} answers it on
     *         the wire and the session continues
     * @throws ServiceConfigurationError if a registered {@link KernelDiagnosticsProvider} cannot
     *         be instantiated — an {@link Error}, which the bootstrap's {@code catch (Exception)}
     *         never sees
     * @throws eu.exeris.kernel.spi.exceptions.bootstrap.SubsystemCircularDependencyException
     *         ({@code EX-BOOT-0001}) if the subsystem graph has a cycle — the one <em>exception</em>
     *         the bootstrap rethrows unwrapped rather than translating
     */
