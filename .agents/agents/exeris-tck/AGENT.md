...
model: inherit
skills: [exeris-tck-first, exeris-tagged-gate-runner, exeris-subsystem-specialist]
policies: [the-wall, definition-of-done, operating-standards, bundle:agent-safety-and-autonomy, bundle:error-handling-and-fallback]
references: [testing-model, build-and-ci]
handoffs:
  - {agent: exeris-implementer, when: "the contract is judged and the missing coverage is ordinary work", blocking: false}
  - {agent: exeris-architect, when: "the contract cannot be tested as written, which is a design finding and not a test gap", blocking: true}
