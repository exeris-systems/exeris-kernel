...
model: inherit
skills: [exeris-architect-guardrails, exeris-service-loader-and-bootstrap, exeris-subsystem-specialist]
policies: [the-wall, scoped-bans, operating-standards, bundle:agent-safety-and-autonomy, bundle:error-handling-and-fallback]
references: [build-and-ci]
handoffs:
  - {agent: exeris-tck, when: "the direction changes observable SPI behaviour", blocking: true}
  - {agent: exeris-implementer, when: "the direction is settled and the change is ready to write", blocking: false}
  - {agent: exeris-docs-adr, when: "the decision needs an ADR, an amendment or a subsystem-contract edit", blocking: false}
