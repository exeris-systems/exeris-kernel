...
model: inherit
skills: [exeris-triage, exeris-doc-impact-triage]
policies: [the-wall, scoped-bans, operating-standards, bundle:agent-safety-and-autonomy]
references: [build-and-ci]
handoffs:
  - {agent: exeris-architect, when: "placement, a module boundary or ADR alignment is the primary risk", blocking: true}
  - {agent: exeris-tck, when: "observable SPI behaviour changes — the contract judge answers before the work is called done", blocking: true}
  - {agent: exeris-implementer, when: "placement and contract are already settled", blocking: false}
  - {agent: exeris-performance, when: "a hot path, an allocation or a native-memory lifetime is in play", blocking: false}
  - {agent: exeris-docs-adr, when: "the change moves what a subsystem contract or an ADR says", blocking: false}
